...
describe("Group Management Functions", () => {
	let groupId;
	let testEmails, testPasswords, testUserNames;
	...
	beforeAll(async () => {
		...
		testUserNames = ["user1", "user2", "user3"];

		for (let i = 0; i < 3; i++) {
			await signup(testEmails[i], testPasswords[i], testUserNames[i]);
		}
	...
	it("should fetch all users in group", async () => {
		...
		expect(data[0].username).toBe(testUserNames[0]);
